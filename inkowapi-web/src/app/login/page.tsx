...
import { ScanFace, Sparkles, Zap, Eye, EyeOff, ShieldCheck, UserPlus } from 'lucide-react'
    ...
    try {
      ...
      if (error) {
        if (error.code === 'USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL') {
          ...
          alert(`IDENTITY PROTOCOL: User '${finalEmail}' already exists. Please Log In.`)
        } else {
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.div
            ...
            className="hidden md:flex absolute top-0 right-0 w-[450px] h-full z-20 flex-col items-center justify-center p-16 text-center overflow-hidden"
            style={{
              background: isLoginView
              ...
              borderLeft: `1px solid ${
                isLoginView ? 'rgba(31, 143, 74, 0.15)' : 'rgba(8, 145, 178, 0.15)'
              }`,
            }}
          >
            {/* Animated glow */}
            <motion.div
              ...
              style={{
                background: isLoginView ? 'rgba(127, 255, 199, 0.3)' : 'rgba(34, 211, 238, 0.25)',
              }}
            />
            ...
            <div className="relative z-10">
              ...
              >
                {isLoginView ? (
                  <ShieldCheck style={{ color: loginColors.accent }} size={42} strokeWidth={1.5} />
                ) : (
                  <UserPlus style={{ color: registerColors.accent }} size={42} strokeWidth={1.5} />
                )}
              </motion.div>
          ...
          <div className="md:hidden flex flex-col items-center justify-center p-8 pb-0 text-center z-20">
            ...
            >
              {isLoginView ? (
                <ShieldCheck style={{ color: loginColors.accent }} size={32} strokeWidth={1.5} />
              ) : (
                <UserPlus style={{ color: registerColors.accent }} size={32} strokeWidth={1.5} />
              )}
            </motion.div>
            <h1
              className="text-3xl font-black tracking-tighter uppercase mb-2 bg-clip-text text-transparent"
              style={{
                backgroundImage: activeColors.gradient,
                WebkitBackgroundClip: 'text',
                backgroundClip: 'text',
          ...
          <div className="relative flex-1 flex flex-col items-center justify-center md:block h-full bg-white/30 backdrop-blur-sm md:bg-transparent">
              ...
              <motion.div className="w-full max-w-sm space-y-8">
                ...
                <div className="text-center md:hidden">
                  <button
                    ...
                    className="text-[10px] uppercase tracking-widest transition-colors font-bold mt-4"
                    style={{ color: '#7A9990' }}
                  >
                    Already have an account?{' '}
                    <span style={{ color: loginColors.accent }}>Login Here</span>
                  </button>
                </div>
              </motion.div>
            </div>
            ...
            {/* LOGIN FORM */}
            <div
              className={`w-full h-full flex items-center justify-center p-6 md:p-20 transition-opacity duration-300 md:absolute md:right-0 md:top-0 md:w-[650px] ${
                !isLoginView
                  ? 'hidden opacity-0 pointer-events-none'
                  : 'flex opacity-100 pointer-events-auto'
              }`}
              ...
              <motion.div className="w-full max-w-sm space-y-8">
                ...
                <div className="text-center md:hidden">
                  ...
                  >
                    ...
                    <span style={{ color: registerColors.accent }}>Register Here</span>
                  </button>
                </div>
              </motion.div>
            </div>
          ...
          <motion.button
            ...
            className="hidden md:flex absolute top-1/2 z-50 w-20 h-20 items-center justify-center cursor-pointer overflow-hidden rounded-full"
            style={{
              right: 410,
              marginTop: -40,
