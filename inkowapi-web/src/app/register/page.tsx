...
import { proxyScanKTP } from '../actions/scanKTP'
import {
  ScanFace,
  ChevronRight,
  ChevronLeft,
...
export default function IdentityVerificationPage() {
  ...
  const matchFromList = (text: string, validList: string[]) => {
    ...
    if (
      ...
      upper === 'SEAM' ||
      upper === 'SIAM' ||
      upper.includes('MOSLEM') ||
      upper.includes('MUSLIM') ||
      /^[I1L|]?S[L1I]?A?M$/i.test(upper) ||
      /ISL.?M/i.test(upper) ||
      /^S[EI]A[MN]$/i.test(upper)
    ) {
      return 'ISLAM'
    }
    if (upper.includes('RISTEN') || upper.includes('KRISTEN') || upper.includes('KR1STEN'))
      return 'KRISTEN'
    if (upper.includes('ATOLIK') || upper.includes('KATOLIK') || upper.includes('KATHOL'))
      return 'KATOLIK'
    if (upper.includes('HINDU') || upper.includes('H1NDU')) return 'HINDU'
    if (upper.includes('BUDDHA') || upper.includes('BUDHA') || upper.includes('BUDDA'))
      return 'BUDDHA'
    if (upper.includes('KONGHUCU') || upper.includes('KHONGHUCU') || upper.includes('KONGHU'))
      return 'KONGHUCU'
  ...
  const handleScan = async (file: File, type: 'ktp' | 'npwp') => {
    ...
    try {
      if (type === 'ktp') {
        ...
        try {
          const result = await proxyScanKTP(formData)

          if (result.success && result.data) {
            const data = result.data
            console.log('DEBUG: Python API response via Proxy:', data)
  ...
  const handleScanWithTesseract = async (file: File) => {
    ...
    if (nameMatch) {
      ...
      n = n
        .replace(/\s+(RT|RW|TA|TH|NIK|JL|GG|NO)\s*$/gi, '')
    ...
    if (maritalMatch) {
      ...
      status = status
        .replace(/\b(SIDOARJO|SIDOARO|SURABAYA|JAKARTA|BANDUNG|SEMARANG|MALANG|YOGYAKARTA)\b/gi, '')
        .trim()
    ...
    const kelMatch = extractedText.match(
      /(?:Kel(?:urahan)?\/Desa|Kel\.|Kelurahan|Desa)\s*[:\s]*([^\n]+)/i,
    )
